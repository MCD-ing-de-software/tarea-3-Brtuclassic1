...
class TestDataCleaner(unittest.TestCase):
    ...
    def test_drop_invalid_rows_removes_rows_with_missing_values(self):
        ...
        self.assertEqual(int(result["name"].isna().sum()), 0)
        self.assertEqual(int(result["age"].isna().sum()), 0)
    ...
    def test_remove_outliers_iqr_removes_extreme_values(self):
        ...
        """
        df = make_sample_df()
        new_row = pd.DataFrame({"name": ["David"], "age": [40], "city": ["SCL"]})
        df = pd.concat([df, new_row], ignore_index=True)

        cleaner = DataCleaner()

        filtered = cleaner.remove_outliers_iqr(df, "age", factor=1.5)

        # verificar que el valor extremo (120) fue eliminado
        self.assertNotIn(120, filtered["age"].tolist())

        # verificar que al menos uno de los valores no extremos (25 o 35) permanece
        self.assertIn(25, filtered["age"].tolist())
