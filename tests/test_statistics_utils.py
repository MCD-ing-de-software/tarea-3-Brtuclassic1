...
class TestStatisticsUtils(unittest.TestCase):
    ...
    def test_zscore_has_mean_zero_and_unit_std(self):
        ...
        # La media de los z-scores debe ser ~0 y la desviación estándar ~1
        self.assertAlmostEqual(z.mean(), 0.0, places=12)
        self.assertAlmostEqual(z.std(), 1.0, places=12)
    ...
    def test_min_max_scale_maps_to_zero_one_range(self):
        ...
        # Propiedades básicas
        self.assertAlmostEqual(scaled.min(), 0.0, places=12)
        self.assertAlmostEqual(scaled.max(), 1.0, places=12)

        # Valores esperados
        expected = np.array([0.0, 0.5, 1.0])
        npt.assert_allclose(scaled, expected, rtol=1e-12, atol=1e-12)
